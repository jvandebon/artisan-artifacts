...
#ifndef __HLS_FLOAT__H__
#define __HLS_FLOAT__H__
...
public:
  ...
  void set_exponent(ac_int<E, false> bits) {
    fp_bit_value.template set_slc<E>(M, bits);
  }

  void set_mantissa(ac_int<M, false> bits) {
    fp_bit_value.template set_slc<M>(0, bits);
  }

  void set_sign_bit(ac_int<1, false> bits) {
    fp_bit_value.template set_slc<1>(E+M, bits);
  }

  void set_bits(ac_int<E + M + 1, false> bits) {
    fp_bit_value = bits;
  }

  ac_int<E, false> constexpr get_exponent() const {
    return fp_bit_value.template slc<E>(M);
  }

  ac_int<M, false> constexpr get_mantissa() const {
    return fp_bit_value.template slc<M>(0);
  }

  ac_int<1, false> constexpr get_sign_bit() const {
    return fp_bit_value.template slc<1>(E+M);
  }

  ac_int<E + M + 1, false> constexpr get_bits() const {
    return fp_bit_value;
  }

  //////////////////////// Commonly Used Constants ////////////////////////

  static constexpr hls_float<E, M, Rnd> pos_inf() {
  ...
  static constexpr hls_float<E,M, Rnd> neg_inf() {
    ac_int<E, false> bit_exponent(0);
    bit_exponent = bit_exponent.bit_complement();
    hls_float<E, M, Rnd> temp;
    temp.set_bits(0);
  ...
  static constexpr hls_float<E,M, Rnd> nan() {
    ac_int<E, false> bit_exponent(0);
    bit_exponent = bit_exponent.bit_complement();
    hls_float<E, M, Rnd> temp;
