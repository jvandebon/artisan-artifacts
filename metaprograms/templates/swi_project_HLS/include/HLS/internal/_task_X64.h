#ifndef _INTEL_IHC_HLS_INTERNAL__TASK_X64
#define _INTEL_IHC_HLS_INTERNAL__TASK_X64
...
namespace ihc {
  namespace internal {
    ...
    template<typename T>
    struct execute {
      T operator()(std::function<T()>& fnc) const {
        return fnc();
      }
    };
    template<>
    struct execute<void> {
      bool operator()(std::function <void()>& fnc) const {
        fnc();
        return true;
      }
    };
    ...
    public:
      ...
      void launch(Args&&... args) {
        auto fnc = new(std::function<T()>){ std::bind(std::forward<F>(f), std::forward<Args>(args)...) };
        // Avoid contention (push/pop)
        {
          std::unique_lock<std::mutex> lock{ _q_fnc_m };
      ...
      T collect() {
        std::unique_lock<std::mutex> _q_ret_lock{ _q_ret_m };
    ...
    private:
      ...
      // MAIN      |-- launch2
      // THREAD2   |     \-----fnc2--
      std::deque < std::function<T()>* > _q_fnc;
      std::mutex _q_fnc_m;               // Access to launch queue
      std::condition_variable _q_fnc_cv; // Notification of of input
                                         // launch->monitor (available)
                                         // ~_task->monitor (flush)
      bool _q_monitor{ true };           // Activate the monitor

      std::deque <R> _q_ret;
      std::mutex _q_ret_m;               // Access to result queue
      std::condition_variable _q_ret_cv; // Notification of output
                                         // monitor->collect
      std::thread _thread{ &_task::monitor, this };
      ...
      void monitor() {
        ...
        R ret;
        std::unique_lock<std::mutex> _q_fnc_lock{ _q_fnc_m, std::defer_lock };
        execute<T> execute;
        while (_q_monitor || !_q_fnc.empty()) {
          ...
          if (!_q_fnc.empty()) {
            ...
            // Avoid contention (push/pop)
            {
              std::lock_guard<std::mutex> q_ret_lock{ _q_ret_m };
...
#endif // _INTEL_IHC_HLS_INTERNAL__TASK_X64
