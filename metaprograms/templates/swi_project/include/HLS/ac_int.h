...
public:
  ...
  template<int W2, bool S2>
  __HLS_CONSTEXPR__ inline void set_val_no_overflow_warning (const ac_int<W2,S2> &op) {
    Base::operator =(op);
  }
...
#pragma clang diagnostic pop
  ...
  inline double to_double() const {
    static_assert(W <= 128, "ac_int to_double() does not support ttype larger than 128 bits");
  ...
  inline ac_int<WS,S> slc(const ac_int<WX,SX> &index) const {
    ...
    ac_int<WS, S> r = 0;
    ac_int<WS, SX> zero = 0;
    AC_ASSERT(index >= zero, "Attempting to read slc with negative indeces");
  ...
  __HLS_CONSTEXPR__ void bit_fill_hex(const char *str) {
...
namespace ac {
  ...
  template<typename T>
  __HLS_CONSTEXPR__ T bit_fill_hex(const char *str) {
    T res;
